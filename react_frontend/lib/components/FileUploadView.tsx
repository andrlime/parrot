        ...
        for(let file of files) {
            ...
            file_reader.onload = (e: ProgressEvent<FileReader>) => {
                ...
                axios.post(`${api_endpoint}/upload`, {
                    ...
                    console.error(error);
                    if (error.response) {
                        dispatch(set_error_msg(`Bad response code (${error.response.status || "not 2xx"}). ${error.response.status == 413 ? "This file is too big." : "Some files will not work. We are not sure the precise reason why."}`));
                        setLoading(false);
